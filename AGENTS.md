...
**目前缺口**：閘門只涵蓋 Python 工具鏈，TypeScript 與 Rust 兩側都零覆蓋。TypeScript 是因為 `bun` 未安裝、`node_modules/` 不存在；Rust 是因為 `cargo check` 在此環境撞 `LNK1104: 無法開啟檔案 'msvcrt.lib'`（`LIB`／`INCLUDE` 未設定，而閘門直接 spawn 命令、不經 shell，無法自行 source `vcvarsall.bat`）。補齊方式見 [`docs/DEVELOPMENT.md`](docs/DEVELOPMENT.md)。
